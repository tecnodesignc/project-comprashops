  ...
  export default {
    ...
    computed: {
      dataLocale() {
        return {
          fields: {
            positionId: this.$route.params.sliderId,
            target: null,
            type: 'auto',
            mediasSingle: {},
            externalImageUrl: '',
      ...
      canManageRecordMaster () {
        let response = true
        if (this.$route.params.sliderId.id && !this.$auth.hasAccess('ibanners.master.records.edit')) {
          response = false
        }
        if (!this.$route.params.sliderId.id && !this.$auth.hasAccess('ibanners.master.records.create')) {
          response = false
        }
        return response
      },
      ...
      async createItem() {
        if (await this.$refs.localeComponent.validateForm()) {
          ...
          this.$crud.create(configName, this.getDataForm()).then(response => {
            ...
            this.$router.push({name: 'qbanner.admin.positions.show', params: {id: this.$route.params.sliderId}})
            this.loading = false
          }).catch(error => {
            this.loading = false
